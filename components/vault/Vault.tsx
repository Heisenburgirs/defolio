      ...
      <div className="flex flex-col py-8 px-6 w-full h-full gap-12">
        {isManage ? (
          <>
            ...
            { !isSettingsUpdating &&
              <div
                className="flex gap-2 px-4 items-center text-lightPurple hover:text-purple hover:cursor-pointer transition text-small z-50"
                onMouseEnter={() => setHover(true)}
                onMouseLeave={() => setHover(false)}
                onClick={() => {settings ? setSettings(false) : isTransfer ? setIsTransfer(false) : setIsManage(false)}}
              >
                <div 
                      ...
                      <div className="flex w-full justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <div 
                            ...
                            onClick={() => {setIndexVault(5), setIndexAsset(5), setIndexKey(5), forceRender(2);}}
                          >
                            <Image 
                    ...
                    <>
                      <div className="flex sm:flex-col md:flex-row sm:gap-4 md:gap-0 w-full justify-between">
                        <div className="flex gap-4 sm:flex-col base:flex-row">
                          ...
                          <div onClick={() => {setIsTransfer(true)}}
                            className="keymanager:hidden text-center py-2 px-4 text-lightPurple border border-lightPurple hover:bg-lightPurple hover:text-white transition hover:cursor-pointer rounded-15">
                              Send & Receive
                          </div>
                        </div>
                        <div className="flex gap-4 sm:flex-col base:flex-row">
                          <button onClick={() => {setIsTransfer(true)}}
                            className="sm:hidden keymanager:flex text-center py-2 px-4 text-lightPurple border border-lightPurple hover:bg-lightPurple hover:text-white transition hover:cursor-pointer rounded-15">
                              Send & Receive
                          </button>
          ...
          addVault ? (
            isDeployingVault ? (
              <TransactionModal
                ...
                message2={isSettingData ? "Setting Data" : isSettingURD ? "Setting URD" : "Deploying Universal Delegate"}
                message3='Transaction Successful'
              />
            )
            :
              ...
              <div className="flex w-full h-full sm:justify-items-center keymanager:justify-items-start grid sm:grid-cols-1 keymanager:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
                {vaults.map((vault, index) => (
                  <div key={index} className="sm:w-[200px] lg:w-[250px] h-[250px] border border-lightPurple rounded-15 py-6 px-4 flex flex-col justify-between">
                    <div className="flex flex-col gap-2">
                      <div className="text-medium text-purple font-bold">{vault.name.substring(0, 12)}</div>
                      ...
                      <div className="text-purple text-xsmall font-bold opacity-75">{vault.desc.substring(0, 55)+"..."}</div>
                    </div>
                    <div 
                      onClick={() => {
                        setIsManage(true);
